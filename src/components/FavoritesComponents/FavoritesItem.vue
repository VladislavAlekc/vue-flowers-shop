<script setup>
const emit = defineEmits(['onClickAdd', 'onClickRemove'])
defineProps({
  id: Number,
  labels: Object,
  imageUrl: String,
...
<template>
  ...
  >
    ...
    <a class="_ibg pb-[103.6%]" href="#">
      <img :src="imageUrl" alt="flowers" />
      <div v-if="labels" class="absolute z-10 bottom-[8px] left-[6px]">
        <div
          :class="labels.type"
          class="leading-[120%] text-[14px] px-[15px] py-[5px] bg-[#bd2597] rounded-[10px] text-white font-bold"
        >
          {{ labels.name }}
        </div>
      </div>
    ...
    <div class="flex flex-auto flex-col p-[12px_10px_12px_10px]">
      <a href="#" class="text-[18px] leading-[120%] font-bold mb-[5px] hover:text-[#368863]">
        {{ title }}
      </a>
      <div class="text-gray-500 leading-[120%] mb-[15px]">{{ size }}см</div>
      <div class="flex items-center mb-[15px]">
        <div class="leading-[120%] text-[20px]">{{ priceFormat.format(price) }}</div>
        <div
          v-if="priceOld"
